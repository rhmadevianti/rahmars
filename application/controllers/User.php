    ...
    public function daftar()
{
    ...
    $this->form_validation->set_rules('dokter_spesialis', 'Dokter Spesialis', 'required');

    if ($this->form_validation->run() == false) {
        $this->load->view('layout/header', $data);
        $this->load->view('layout/topbar');
        $this->load->view('layout/sidebar');
        $this->load->view('user/daftar_berobat', $data);
        $this->load->view('layout/footer');
    } else {
        $data = [
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-success neu-brutalism">Pendaftaran berhasil dikirim. Silakan cek status secara berkala.</div>');
        redirect('user/status'); // ganti ke 'pasien/status' jika controller sudah diganti
    }
}
